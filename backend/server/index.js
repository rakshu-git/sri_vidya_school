...
app.post('/api/send-reminder', async (req, res) => {
    ...
    try {
        ...
        if (email) {
            const mailOptions = {
                from: '"Sri Vidya School" <reminders@school.com>',
                to: email,
                subject: 'Monthly Donation Reminder',
                text: `Namaste ${name},\n\nIt has been ${daysSinceLastDonation} days since your last donation. Your contribution helps us maintain the school and serve the community.\n\nVisit your dashboard to make a donation: ${process.env.FRONTEND_URL || 'http://localhost:5173'}/dashboard\n\nBlessings,\nSri Vidya School`,
                html: `<div style="font-family: sans-serif; padding: 20px; border: 1px solid #ffedd5; border-radius: 10px;">
                    ...
                    <p>Your contribution helps us maintain the school and mould the childrens future.</p>
                    ...
                    <p style="margin-top: 20px; color: #666;">Blessings,<br>Sri Vidya School</p>
                </div>`
            };
        ...
        // Send SMS
        if (phone) {
            const smsMessage = `Namaste ${name}, it has been a month since your last donation to the School. Your support means a lot. Donate here: ${process.env.FRONTEND_URL || 'http://localhost:5173'}/dashboard`;
...
app.post('/api/test-email', async (req, res) => {
    ...
    try {
        ...
        const info = await transporter.sendMail({
            from: `"Sri Vidya School" <${process.env.SMTP_USER}>`,
            to: email,
            subject: 'Monthly Donation Reminder - Test',
            text: `Namaste ${name},\n\nIt has been a month since your last donation. Your contribution helps us maintain the school and mould future of childrens.\n\nVisit your dashboard to make a donation: ${process.env.FRONTEND_URL || 'http://localhost:5173'}/dashboard\n\nBlessings,\nSri Vidya School`,
            html: `<div style="font-family: sans-serif; padding: 20px; border: 1px solid #ffedd5; border-radius: 10px;">
                <h2 style="color: #ea580c;">Namaste ${name},</h2>
                <p>It has been <b>a month</b> since your last donation.</p>
                <p>Your contribution helps us maintain the school and mould future of childrens.</p>
                <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/dashboard" style="display: inline-block; background: #ea580c; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;">Donate Now</a>
                <p style="margin-top: 20px; color: #666;">Blessings,<br>Sri Vidya School</p>
            </div>`
        });
...
app.post('/api/test-sms', async (req, res) => {
    ...
    try {
        const msg = `Namaste ${name}, this is a test SMS from your Sri Vidya School. It works!`;
...
app.listen(PORT, HOST, () => {
    console.log(`School Backend running at http://${HOST}:${PORT}`);
